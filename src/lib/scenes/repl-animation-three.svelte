<script>
	...
	import { HTML, GLTF, useGltfAnimations } from '@threlte/extras';
	...
	function init() {
		...
		loader.load(url, function (gltf) {
			...
			threeActions = [idleAction, walkAction, runAction];
	...
	function createPanel() {
		...
		const folder7 = panel.addFolder('Threlte crossFade');
		...
		folder7.add(
			{
				'threlteCross': () => {
					console.log("threlte cross");
					prepareCrossFade($actions['Walk'], $actions['Idle'], 0.5);
				}
			}, 'threlteCross'
		)
	...
	function deactivateAllActions() {
		threeActions.forEach(function (action) {
			action.stop();
		});
	}
...
<PerspectiveCamera position={{ x: 0.3, y: 1.7, z: -2.3 }} fov={90}>
	<OrbitControls target={{ y: 0.75 }} maxPolarAngle={85 * DEG2RAD} />
</PerspectiveCamera>
